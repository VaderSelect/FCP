...
# --- Настройки ---
BOT_TOKEN = "ВАШ_ТОКЕН_ОТ_BOT_FATHER"  # Замените на ваш токен
...
async def handle_file_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if '1 неделя' in file_name or '1-я' in file_name:
        target_file = FILE_1
    elif '2 неделя' in file_name or '2-я' in file_name:
        target_file = FILE_2
    else:
        # Если не понятно, просим уточнить
        await update.message.reply_text(
...
async def scheduled_update():
    """Функция для автоматического обновления расписания."""
    while True:
        # Обновляем каждый день в 4:00
        now = datetime.now()
        if now.hour == 4 and now.minute == 0:
            logger.info("Запуск автоматического обновления расписания...")
            update_schedule()
            load_schedule()
...
def main():
    ...
    # Запускаем фоновую задачу для автообновления
    # application.job_queue.run_repeating(scheduled_update, interval=60, first=10)
